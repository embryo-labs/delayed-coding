//! Research-only exact DC16, interleaved in SIMD groups of sixteen states.
//! No logarithmic approximation. Not linked into the supported library.
...
impl SimdModel {
    ...
    fn build(frequencies: &[u32], cumulative: bool) -> Result<Self, Error> {
        ...
        let size = if compact { 4096 } else { 65536 };
    ...
    pub fn encode64_into(
        ...
        let bytes = self.encode::<64>(input)?;
        let start = output
            .len()
